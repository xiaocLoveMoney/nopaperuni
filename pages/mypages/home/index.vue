<template>
	<view style="background-color: #EAEDF1;">
		<scroll-view @scroll="handleScroll" style="height: 100vh;" scroll-y>
			...
			<u-card style="margin: 5%;" :title="'个人基本信息'">
				<view slot="body">
					<u-row>
	...
	export default {
		...
		methods: {
			...
			async switch_btn(path) {
				...
				const index = this.info.role_names.indexOf(e => e == '学生')

				if (index != -1 && path == "/pages/mypages/home/createLeaves") {
					setTimeout(function() {
						uni.navigateTo({
							url: path
						})
					}, 500);
				} else if (index == -1 && path != "/pages/mypages/home/createLeaves") {
					if (path == '/pages/mypages/home/uploadAttendance') {
						this.$u.get(`/api/analysis/tablehelper1?date=${this.get_date_format(new Date())}`).then(res =>{
							uni.showToast({
								title: '已提交,请勿重复'
							});
						}).catch(err => {
